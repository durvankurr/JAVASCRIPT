...
Ron has hens, ducks, and cows in the barn that he wants to renovate. He believes that erecting fences to keep the animals apart will make it easier for him to control them.​
...
1. the required functions exist ​
2. return expected results for the valid inputs provided ​
3. return the error messages for the insufficient / invalid and incorrect inputs provided.​
...
1. Open the file `calculate-area-solution.js` located inside the `solution` folder of the `fe-c2-s4-testing-practice` folder to copy the solution.​
2. Copy the solution code for this practice from the solution code developed for the `practice 1` of `Sprint 3 - Implement Modular Programming using Functions` into the `calculate-area-solution.js` file .
...
3. Switch to command terminal and set the path to the folder `fe-c2-s4-testing-practice`.
...
5. Run the command `npm install mocha chai` to install the testing dependencies `Mocha` and `Chai`.
...
1. Open the file `calculate-area-spec.js` located inside the folder `test` of the folder `fe-c2-s4-testing-practice`.​
2. Write test case to test whether the definition of the function `areaOfSquare()` exists.
3. To execute the test case, from the command terminal, run the command​ 
...
6. Execute the test case as explained in Step 3 and if required, refactor the code to ensure that the test cases pass.
...
- To check for non-zero or non-negative input:
    - Use `if` statement to ensure that the value of the parameter is not less than or equal to zero.​
...
1: Inside the file `calculate-area-spec.js` located inside the folder `test` of the folder `fe-c2-s4-testing-practice`, write test cases to test whether the function `areaOfRectangle()`
- is defined.​
...
1. Inside the file `calculate-area-spec.js` located inside the folder `test` of the folder `fe-c2-s4-testing-practice`, write test cases to test whether the function `areaOfCircle()` ​
